...
1. `POST /v1/events`
   ...
   - Stores one `inbound_events` row for a new idempotency key.
...
5. The route handler validates the body and required `Idempotency-Key` header.
6. The handler checks whether an inbound event already exists for that idempotency key.
7. If it is new, the handler creates an `InboundEvent` row.
8. The handler appends two audit events inside the same transaction:
   - `event_received`, attributed to `api_client`
   - `event_accepted`, attributed to `system`
9. The transaction commits.
10. The response includes the same correlation ID, and the middleware also writes it to the `X-Correlation-ID` response header.

For a duplicate inbound event retry:

1. The API client calls `POST /v1/events` again with the same `Idempotency-Key`.
2. The service finds the existing inbound event.
3. The service returns the existing event ID.
4. The service does not create another inbound event.
5. The service does not append another audit trail for the duplicate request.
...
A retry may have a new correlation ID but the same idempotency key. That is acceptable. The correlation ID tracks the retry request; the idempotency key links the retry to the same logical work item.
...
### Keep duplicate idempotency requests quiet in the audit trail

When `POST /v1/events` receives a duplicate `Idempotency-Key`, it returns the existing event ID but does not append another audit event.
